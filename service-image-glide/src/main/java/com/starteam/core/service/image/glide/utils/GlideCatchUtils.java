...
public class GlideCatchUtils {
    ...
    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(
                    AppContextUtils.getApp().getCacheDir() + "/" + GlideConfig.GLIDE_CATCH_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    public boolean cleanCatchDisk() {
        return deleteFolderFile(AppContextUtils.getApp().getCacheDir()
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(AppContextUtils.getApp()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(AppContextUtils.getApp()).clearDiskCache();
    ...
    public boolean clearCacheMemory() {
        try {
            //只能在主线程执行
            if (Looper.myLooper() == Looper.getMainLooper()) {
                Glide.get(AppContextUtils.getApp()).clearMemory();
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
